...
For this analysis we are using our merged data from enrolment and archetype dataset.Although we don't have archetype for batch 1 and batch 2, but here we are merging all the available data that we have and then producing a broad overview of our analysis.
...
```{r, echo = FALSE,out.width="100%", fig.cap="Weekly feedback about the course"}
...
For carrying out analysis for the continents for maximum number of course videos watched by the learners, which in a way shows the maximum participation from the continents, we used the merged video_stats dataset from all the batches.
...
```{r, echo = FALSE,out.width="100%", fig.cap="Learners from the countries across the world"}
knitr::include_graphics("C:/Users/Payal/Desktop/Future_Learn_EDA_DM/graphs/countrywise_enrolments.png")
```


Here we see that the enrolments from the top 10 countries follow the trend of the learners enrolled from respective continents. This tells us that our data is reliable.
...
** Explain the sum of all the individual devices taken ** -- to be included
...
Fig 8. gives the number of devices udes for watching the video modules of the course 
